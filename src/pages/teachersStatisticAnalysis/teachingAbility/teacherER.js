...
function TeacherER(props, ref) {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return `<div  class="t-tooltip">
            ...
            data[3]
          }人</span></p><p class='msg msg-2'>未参与上传人数<span>${
            data[2] - data[3]
          }人</span></p></div>
            `;
        },
  ...
  return (
    <div className={`teacher-bar TeacherER ${className ? className : ""} `}>
      <div className="ter-left">
        <p className="tb-tip">
          {productMsg && productMsg.title ? productMsg.title : ""}
          总体电子资源上传<span className="tb-tip-2">{ResourceCount}</span>
          份，人均上传
          <span className="tb-tip-2">{AvgUploadResourceCount}</span>
          份，平均每日有
          <span className="tb-tip-2">{AvgUploadUserCount}</span>人参与上传
        </p>
...
export default connect(mapStateToProps)(memo(forwardRef(TeacherER)));
